...
const Wxaudio = function (options) {
...
Wxaudio.prototype = {
	...
	initDom: function () {
		...
		this.wxAudioT.className = 'middleX';
		...
		// left image
		this.wxAudioStateImg = document.createElement('i');
		this.wxAudioStateImg.className = 'iconfont icon-bofang';
	...
	initAudioEvent: function () {
		...
		_this.wxAudio.onplaying = function () {
			...
			_this.wxAudioStateImg.className = 'iconfont icon-zanting';
		},
			_this.wxAudio.onpause = function () {
				_this.isPlaying = false;
				// _this.showLoading(false)
				_this.wxAudioStateImg.className = 'iconfont icon-bofang';
			},
		...
		_this.wxAudioOrigin.ontouchstart = function (event) {
			...
			_this.wxAudioC.ontouchmove = function (event) {
				if (_this.isDrag) {
					...
					const thisX = e.touches[0].clientX;
					_this.dragProgressTo = Math.min(_this.maxProgressWidth, Math.max(0, l + (thisX - x)));
					_this.updatePorgress()
				}
			},
